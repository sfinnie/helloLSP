...
**Aside** The LSP provides separate features for go to *declaration* and go to *definition*.  **TODO**: does LSP demarcate thw two anywhere??
...
<a name="extended-greet-example"></a>
...
1. both `definition` and `greeting` refer to the same term on the last line of the grammar.  That makes sense: we don't want different rules for what constitutes a valid name when defining it vs using it.
...
The grammar only specifies the language *syntax*.  It doesn't say anything about its *semantics* - its *meaning*.  We haven't had to deal with semantics up to this point.  Our regex implementation of the original grammar was sufficient.  As humans reading the language, we understand it would break our new rules to greet someone whose name hasn't been defined.  We need to fnd a way to encode that.
...
We could hand-write a parser.  But we could also use a [parser-generator]() that takes a definition of our grammar and creates a parser for us.  There are good reasons to use either approach; we'll use the latter.  There are many parser-generators available.  I've chosen to use [tree-sitter](https://tree-sitter.github.io/) for a number of reasons:

1. It's fast. It was originally created for the [Atom]() editor, specifically for parsing and syntax highlighting programming languages.
